...
/*   Updated: 2025/12/02 16:29:25 by jarregui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_args(int argc, char **argv)
{
	...
	if (argc < 5 || argc > 6)
	{
		printf("\033[0;91mERROR - You must enter 4 arguments:\n");
...
static void	init_rules(int argc, char **argv, t_rules *rules)
{
	int	i;
...
static void	start_threads(t_rules *rules, t_philo *philos)
{
	int			i;
	pthread_t	monitor;
	...
	while (i < rules->num_philosophers)
	{
		...
		if (pthread_create(&philos[i].thread, NULL,
			...
			error_message("Thread creation failed", 1);
		i++;
	}
...
int	parse_args_and_start_philos(int argc, char **argv, t_rules *rules)
{
	...
	if (!philos)
		error_message("Malloc failed", 1);
	init_philos(rules, philos);
	start_threads(rules, philos);
	i = 0;
	while (i < rules->num_philosophers)
